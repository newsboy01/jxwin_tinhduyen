...
void KPlayerChat::SendMsgSystem(int nIdPlayer, char *nMsgValue)
{
	if (!nMsgValue || !nIdPlayer || nIdPlayer == 0)
		return;

	SEND_MSG_SYSTEM_SYNC sSMSS;
	sSMSS.ProtocolType = s2c_sendsyncmsgsystemvalue;
	sSMSS.m_nPlayerIdx = nIdPlayer;
	strncpy(sSMSS.msgconten, nMsgValue, sizeof(sSMSS.msgconten) - 1);
	sSMSS.msgconten[sizeof(sSMSS.msgconten) - 1] = '\0'; // dam bao chuoi ket thuc bang '\0'
	g_pServer->PackDataToClient(Player[nIdPlayer].m_nNetConnectIdx, (BYTE *)&sSMSS, sizeof(SEND_MSG_SYSTEM_SYNC));
}
#endif

// trungnh SendMsgSystem LuaMsgToPlayerSystem MsgToPlayerST
#ifndef _SERVER
void KPlayerChat::SendMsgSystem(char *nMsgValue)
{
	if (!nMsgValue)
		return;
	KSystemMessage sMsg;
	sprintf(sMsg.szMessage, "%s", nMsgValue);
...
//---------------------------------------------------------------------------
// Ch?c n?ng: G?i tin nh?n h? th?ng
// nType == 0 ???c g?i t?i t?t c? ng??i ch?i nType == 1 ???c g?i t?i m?t ng??i ch?i c? th?
// dwTargetID: if (nType == 1) V? trí c?a ng??i ch?i m?c tiêu trong m?ng ng??i ch?i
// lpszSendName: Tên ng??i g?i, có th? là tin nh?n h? th?ng, th?ng báo, c?nh báo, chú ?, v.v. ?? dài kh?ng v??t quá 32
// lpszSentence: C?u c?n g?i
// nSentenceLength: ?? dài c?a c?u c?n g?i
//---------------------------------------------------------------------------
#ifdef _SERVER
...
void KPlayerChat::SendSystemInfo(int nType, int nTargetIdx, char *lpszSendName, char *lpszSentence, int nSentenceLength)
{
	...
	else if (nType == 103) // trungnh SendMsgSystem LuaMsgToPlayerSystem MsgToPlayerST
	{
		if (nTargetIdx > 0)
		{
			int nTargetIdx = PlayerSet.GetFirstPlayer();
			while (nTargetIdx)
			{
				// Player[nTargetIdx].m_cChat.AddMsgSystem((char *)lpszSentence);
				// KPlayerChat &chat = Player[nTargetIdx].m_cChat;
				// chat.AddMsgSystem((char *)lpszSentence);
				// AddMsgSystem((char *)lpszSentence);
				nTargetIdx = PlayerSet.GetNextPlayer();
			}
		}
	}
