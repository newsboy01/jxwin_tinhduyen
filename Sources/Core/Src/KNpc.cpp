...
void KNpc::Init()
{
	...
	m_SkillParam2 = 0;
	m_TypeNpc = 0;
	m_nNpcTimeout = 0;//trungnh SetNpcTimeOut
...
//	    g_DebugLog("den gio xoa");
		...
		if (ActionScript)
		{
			NpcSet.ExecuteScript(m_Index, ActionScript, "Timeout", m_Index);
...
void KNpc::OnDeath()
{
	if (WaitForFrame())
	{
		...
		if (IsPlayer() && Npc[m_nLastDamageIdx].IsPlayer())
		{
			...
			if (m_nPlayerIdx > 0)
			{
				sprintf(szMessageDeath,"B¹n ®· b?%s ®¸nh träng th­¬ng",Npc[m_nLastDamageIdx].Name);
...
//xxx			
			...
			if (nPKValue > 0)
			{
				...
				if (nPKValue == 10)
				{
					...
					sprintf(ThongBaoDoBen,"Toµn b?trang b?mÆc trªn ng­êi ®· háng, vui lßng ®Õn Th?RÌn ®Ó phôc håi");
				...
				else
				{
					...
					char ThongBaoDoBen[100];
					sprintf(ThongBaoDoBen,"§é bÒn toµn b?trang b?gi¶m %d phÇn tr¨m do b¹n ®ang c?%d ®iÓm PK trong ng­êi",dobengiam,nPKValue);
					KPlayerChat::SendSystemInfo(1,m_nPlayerIdx, MESSAGE_SYSTEM_ANNOUCE_HEAD,ThongBaoDoBen,strlen(ThongBaoDoBen) );
				}
...
GetLocalTime(&TimeNow);
		...
		nRateMoney = m_pDropRate->nMoneyRate;
	       ...
	       if (m_IsBoss)
			{
				Player[nBelongPlayer].ExecuteScript("\\script\\addnpccacmap\\addquai\\bossxanh.lua","OnDeath",0);
