...
KUiCheckItemBox* KUiCheckItemBox::OpenWindow(char* szTitleBox,char* szTitleTex)
{
	...
	if (m_pSelf)
	{
		...
		//trungnh lockmove GOI_LOCKMOVE
        if (g_pCoreShell)
            g_pCoreShell->OperationRequest(GOI_LOCKMOVE, TRUE, pos_checkbox);
...
void KUiCheckItemBox::CloseWindow()
{
	if (m_pSelf)
	{
		...
		m_pSelf->Destroy();
		m_pSelf = NULL;
        g_pCoreShell->OperationRequest(GOI_LOCKMOVE, 0, 0);//trungnh lockmove
	}
}
...
int KUiCheckItemBox::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WND_N_BUTTON_CLICK:
	    ...
	    else if (uParam == (unsigned int)(KWndWindow*)&m_OkBtn)
		{
			 if (g_pCoreShell)
			 {
				g_pCoreShell->OperationRequest(GOI_CHECK_ITEM_BOX, 0, 0); //trungnh
			 }
			 CloseWindow();
		}
		break;
