...
#include "UiCase/UiAuto.h" //trungnh auto phim f
...
#define	UI_AUTOEXEC_SETTING_FILE	"\\autoexec.lua"
...
int LuaSwitchStatus(Lua_State * L)
{
	...
	if (strStatus)
	{
        ...
        case 13:    //显示玩家生命
            if (g_pCoreShell)
            {
				KUiAuto::ClickButtonAutoPlay(); //trungnh auto phim f
				/*
				if (g_pCoreShell->GetAutoFlag())
				{
					if (g_pCoreShell->GetFightFlag())
					{
						g_pCoreShell->OperationRequest(GOI_AUTO_PLAY,5,FALSE);// trungnh goi nut tu danh
					}
					else
						g_pCoreShell->OperationRequest(GOI_AUTO_PLAY,5,TRUE);// trungnh goi nut tu danh
				}
				*/
            }
            break;
