...
int KLogin::SelectRole(int nIndex)
{
	...
	if (file != NULL)
	{
		fprintf(file, "m_Status: %d, LL_S_ROLE_LIST_READY: %d, nIndex: %d, m_nNumRole: %d, m_Result: %d\n", m_Status, LL_S_ROLE_LIST_READY, nIndex, m_nNumRole, m_Result);
		// fclose(file);
	}
	*/
	...
	if (m_Status == LL_S_ROLE_LIST_READY && nIndex >= 0 && nIndex < m_nNumRole)
	{
		...
		if (file != NULL)
		{
			fprintf(file, "NetCommand.szRoleName: %d, m_Choices.szProcessingRoleName: %d\n", NetCommand.szRoleName, m_Choices.szProcessingRoleName);
			fclose(file);
		}
		*/
	}
...
void KLogin::ProcessToLoginGameServResponse(tagNotifyPlayerLogin *pResponse)
{
	...
	{
		// trungnh debug client
		/*
		int i;
		FILE* file = fopen("output.txt", "a");
		if (file != NULL)
		{
			fprintf(file, "m_Slect: %d,szRoleName: %d, pResponse->szRoleName: %d, m_Choices: %d \n", m_Slect, *pResponse,(const char*)pResponse->szRoleName, m_Choices.szProcessingRoleName);
			//fclose(file);
		}
		fprintf(file, "Chuoi 1: ");
		for (i = 0; pResponse->szRoleName[i] != '\0'; ++i) {
			fprintf(file, "%c ", pResponse->szRoleName[i]); // In t?ng k? t? c?a chu?i
		}
		fprintf(file, "\n");

		fprintf(file, "Chuoi 2: ");
		for (i = 0; m_Choices.szProcessingRoleName[i] != '\0'; ++i) {
			fprintf(file, "%c ", m_Choices.szProcessingRoleName[i]); // In t?ng k? t? c?a chu?i
		}
		fprintf(file, "\n");
		*/
