	...
	if (m_bIsGameServConnecting && m_pGameSvrClient)
	{
		...
		while (true)
		{
			...
			while (pMsg < (PROTOCOL_MSG_TYPE *)(pBuffer + nSize))
			{
				...
				{
					_ASSERT(Msg > s2c_clientbegin && Msg < s2c_end);  // trungnh bao loi cho nay
					_ASSERT(g_pCoreShell->GetProtocolSize(Msg) != 0); // trungnh bao loi cho nay
...
#define MSG_EXCHANGE_FAIL "Ph輆 tr?韈 ??阯g kh玭g th玭g v蒼 s? ?課 m閠 l莕 n鱝!"
		KSystemMessage Msg;
